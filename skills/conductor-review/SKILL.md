...
2.  **Auto-Detect Scope:**
    ...
    -   If one exists, ask the user to confirm:
        > Do you want to review the in-progress track '<track_name>'? (Yes/No)
        Wait for the user's response.
    -   If no track is in progress, or user says "no", ask the user for the scope:
        > What would you like to review? (Enter track name, or 'current' for uncommitted changes)
        Wait for the user's response.
3.  **Confirm Scope:** Ensure you and the user agree on what is being reviewed by asking:
...
3.  **Load and Analyze Changes (Smart Chunking):**
    ...
    -   **Strategy Selection:**
        ...
        -   **Large Changes (> 300 lines):**
            -   **Confirm:** Ask the user to confirm before proceeding with a large review:
...
2.  **Condition for Action:**
    ...
    -   If changes are detected:
        a. **Check for Track Context:**
            - If you are NOT reviewing a specific track (i.e., you don't have a `plan.md` in context), ask the user:
                > I've detected uncommitted changes. Should I commit them? (Yes/No)
                Wait for response. If 'yes', stage all changes and commit with `fix(conductor): Apply review suggestions <brief description of changes>`.
                - Proceed to '3.3 Track Cleanup'.
        b. **Handle Track-Specific Changes:**
            i.   **Confirm with User:** Ask the user:
...
2.  **Ask for User Choice:** Ask the user:
    > Review complete. What would you like to do with track '<track_name>'?
    > A) Archive (Move track's folder to `conductor/archive/` and remove from tracks file)
    > B) Delete (Permanently delete track's folder and remove from tracks file)
...
3.  **Handle User Response:**
    ...
    *   **If "Delete":**
        i.   **Confirm:** Ask the user for final confirmation:
            > WARNING: This is an irreversible deletion. Do you want to proceed? (Yes/No)
            Wait for response.
